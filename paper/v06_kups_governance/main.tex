...
landed inside the strict band as a fortuitous cancellation of four errors --- three in the calculation
(silicon double-count, under-converged cutoff, non-calibration structure) and one in the reference
(a mis-derived $K_H$); rebuilt clean (pure Talu oxygen-only,
converged convention, on Talu's calibration Olson geometry) it reproduces the corrected reference
$K_H = 0.200$ exactly and cross-engine (native 0.200, RASPA3 0.199). \textbf{The verdict does not
change} --- the as-locked recipe is inside the strict band against both the mis-derived 0.224
($\Delta\log_{10}K_H = -0.034$) and the corrected 0.200 ($+0.015$), and the three computational errors
net to only $\times1.04$; what the rebuild changes is the \emph{coherence} of the recipe, not its
pass/fail. The clean rebuild is in fact slightly worse on $Q_{st}$ ($-0.73$ vs $+0.33$ kJ/mol), which is
what a spurious-attraction cancellation predicts. Separately: per public-source searches conducted on
15 June 2026 we are not aware of a prior external check of kUPS's Widom estimator, which we check here
against two independent estimators (isosteric
heat reproduced to 0.01--0.22 kJ/mol on two locked controls; native $\leftrightarrow$ RASPA3 agree on the
shared recipe to 1.1\% on $K_H$, while kUPS differs by $+16$\% until its truncation convention is matched
and by 2.4\% once it is; the dominant $K_H$ convention gaps traced to named truncation/shift
...
\begin{figure}[h!]\centering\includegraphics[width=0.95\linewidth]{f1_wpb_triptych.pdf}
\caption{WPB --- governance discriminates the failure mode across four configurations (Si-CHA + Ar).}
\end{figure}
\begin{figure}[h!]\centering\includegraphics[width=0.80\linewidth]{fp_6c_cancellation.pdf}
...
\end{figure}
\begin{figure}[h!]\centering\includegraphics[width=0.98\linewidth]{fp_cross_engine_parity.pdf}
...
\begin{figure}[h!]\centering\includegraphics[width=0.98\linewidth]{f2_uma_domain.pdf}
...
\end{figure}
\begin{figure}[h!]\centering\includegraphics[width=0.95\linewidth]{f3a_insertion_map.pdf}
\caption{Where the spurious weight lives: flagged (OOD) insertions sit inside the framework walls.}
\end{figure}
\FloatBarrier\clearpage
\begin{figure}[h!]\centering\includegraphics[width=0.62\linewidth]{f3b_pathology.pdf}
\caption{Pathology close-up: UMA's spurious deep well ($-292.8$ kJ/mol) at a 2.65 \AA{} atomic overlap.}
\end{figure}
\begin{figure}[h!]\centering\includegraphics[width=0.62\linewidth]{f3c_indomain_well.pdf}
