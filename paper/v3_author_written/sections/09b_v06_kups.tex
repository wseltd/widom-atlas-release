...
within \(0.7\) kJ/mol of the calorimetric value. This is the paper's own thesis landing in its own
...
independent estimators agree on a \emph{common} recipe (they do, to \(1.1\%\) on \(K_H\) and \(0.22\)
kJ/mol on \(Q_{st}\)), not whether that recipe reproduces experiment. The experimental-reproduction
...
direct-air-capture head, used on CO\(_2\) in the same framework, its actual training domain, is
...
\caption{Governance respects the training domain. Left: the UMA direct-air-capture head screen-passes
in-domain on CO\(_2\) (a physical well) and is withheld out-of-domain on Ar (internally inconsistent).
Right: the full task-by-system verdict matrix; the model passes exactly where its training domain says
it should.}
\label{fig:v06-uma}
\end{figure}
